...
public abstract class Player extends Robot {
  ...
  protected void getUpIfNecessary() {
    ...
    double pressureReading = Math.abs(leftFootSensor[0]) + Math.abs(leftFootSensor[1]) + Math.abs(leftFootSensor[2]) +
                          + Math.abs(rightFootSensor[0]) + Math.abs(rightFootSensor[1]) + Math.abs(rightFootSensor[2]);
